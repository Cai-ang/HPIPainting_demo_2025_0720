    ...
    public class PokeInteractor : PointerInteractor<PokeInteractor, PokeInteractable>
    {
        ...
        protected override void InteractableSelected(PokeInteractable interactable)
        {
            if (interactable != null && GetBackingHit(interactable, out SurfaceHit backingHit))
            {
                ...
                // // 获取 _selectedInteractable 以及所有子物体上的 ColorPickClick 组件
                // ColorPickClick[] colorPickClicks = _selectedInteractable.GetComponentsInChildren<ColorPickClick>();
                // foreach (ColorPickClick colorPickClick in colorPickClicks)
                // {
                //     if (colorPickClick)
                //         colorPickClick.ClickPoint = _targetTouchPointLocal;
                // }
        ...
        protected override void DoSelectUpdate()
        {
            ...
            //当手指接触到画布时，获取local坐标
            // 获取 _selectedInteractable 以及所有子物体上的 ColorPickClick 组件
            //Debug.Log(_selectedInteractable.name + ":" + _selectedInteractable.tag);
            if (_selectedInteractable.tag == "Menu")
            {
                ColorPickClick[] colorPickClicks = _selectedInteractable.GetComponentsInChildren<ColorPickClick>();
                foreach (ColorPickClick colorPickClick in colorPickClicks)
                {

...
namespace Oculus.Interaction.HandGrab
{
        ...
        IHandGrabInteractor, IRigidbodyRef
    {
        ...
        public GameObject handSyntheticleft;
        public GameObject handSyntheticright;
        ...
        protected override void DoSelectUpdate()
        {
            ...
            //判断是否抓住需要绘制
            if (SelectedInteractable.tag == "pen")
            {
                if (handvisualleft.activeSelf == true)
                {
                    handvisualleft.SetActive(false);
                    handvisualright.SetActive(false);
                    handSyntheticleft.SetActive(true);
                    handSyntheticright.SetActive(true);
                }
                if (SelectedInteractable.handTransform != null)
                {
                    GameObject handvisual = SelectedInteractable.handTransform.gameObject;
                    handvisual.SetActive(true);
                }
                else
                {
                    Debug.LogError("Child object named 'Ghost-RightHand(Clone)' not found.");
                }

                HandFingerFlags fingers = this.GrabbingFingers(SelectedInteractable);
                Pen_3D pen = SelectedInteractable.GetComponentInParent<Pen_3D>();
                //Debug.Log(fingers);
                if (fingers == HandFingerFlags.All)
                    pen.penState = "All";
                // 检查中指和无名指是否同时触发
                else if ((fingers & (HandFingerFlags.Middle | HandFingerFlags.Ring)) == (HandFingerFlags.Middle | HandFingerFlags.Ring))
                {
                    pen.penState = "M&R"; // 或其他表示中指和无名指同时触发的状态
                }
                else if ((fingers & HandFingerFlags.Ring) == HandFingerFlags.Ring)
                {
                    pen.penState = "R";
                }
                else if ((fingers & HandFingerFlags.Middle) == HandFingerFlags.Middle)
                    pen.penState = "M";
                else
                {
                    pen.penState = "G";
            ...
            //换手的渲染方法
            else if (SelectedInteractable.transform.parent.tag == "canScale" && handvisualleft.activeSelf == false)
            {
                handvisualleft.SetActive(true);
                handvisualright.SetActive(true);
                handSyntheticleft.SetActive(false);
                handSyntheticright.SetActive(false);
            }
            //Debug.Log(SelectedInteractable.transform.parent.tag);
        }

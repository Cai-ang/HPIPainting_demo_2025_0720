...
namespace Oculus.Interaction
{
    ...
    public class MaterialPropertyBlockEditor : MonoBehaviour
    {
        ...
        public void Reset()
        {
            ...
            //}
            // 将新渲染器添加到列表中
            List<MeshRenderer> renderer = GetComponentsInChildren<MeshRenderer>().ToList();
            //Debug.Log(renderer.gameObject.name);
            foreach (var renderer0 in renderer)
            {
                _renderers.Add(renderer0 as Renderer);
            }
            //_renderers.Add(renderer as Renderer);
            // 应用更改
            //Debug.Log("_renderers" + _renderers[0].name);

...
namespace pipegenaration
{
    ...
    public class LineCreateOf3D
    {
        ...
        public GameObject CreateControlLine(GameObject mesh, Vector3 startPoint, Vector3 endPoint, int circularCount, float circularR)
        {
            MeshFilter filter = mesh.GetComponent<MeshFilter>();
            if (filter == null)
            {
            ...
            // 创建起始点
            AddPipePoints(startPoint, (endPoint - startPoint).normalized, ref pipePoints);

            // 创建终点
            AddPipePoints(endPoint, (endPoint - startPoint).normalized, ref pipePoints);
